---
title: "MLInterfaces: Machine Learning with Bioconductor for Genomics"
shorttitle: "outlier dytection methods"
author: "Vincent J. Carey, stvjc at channing.harvard.edu, Jess Mar, Robert Gentleman"
date: "`r format(Sys.time(), '%B %d, %Y')`"
vignette: >
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteIndexEntry{MLInterfaces: Machine Learning with Bioconductor for Genomics}
...
Our objective in `r BiocStyle::Biocpkg("MLInterfaces")` is to simplify
...
from Golub's data.
```{r setupp, results="hide", echo=FALSE}
...
the target procedure in package `r BiocStyle::CRANpkg("class")`.
...
to define a cross-validation of a learning process.

```{r eval=FALSE,echo=FALSE}
summary(RObject(nns))
```
%which is the customary `nnet` summary.  This also gives
...
`MLInterfaces` defines 10 S4 classes to help with
data input to diverse learning packages/functions
and to structure outputs of various learning tools.


<!--
classes = c("learnerSchema", "clusteringSchema", "classifierOutput",
"prcompObj", "xvalSpec", "clusteringOutput", "raboostCont", "daboostCont",
"projectedLearner", "varImpStruct")
-->

<!--
../../R/AllClasses.R:## setClass("learnerSchema", representation(
../../R/AllClasses.R:setClass("learnerSchema",
../../R/AllClasses.R:#setClass("clusteringSchema", representation(distMethod="character", 
../../R/AllClasses.R:setClass("clusteringSchema", representation(
../../R/AllClasses.R:setClass("classifierOutput", representation(
../../R/AllClasses.R:#setClass("nonstandardLearnerSchema", representation(frontConverter="function",
../../R/AllClasses.R:setClass("prcompObj", contains="prcomp")
../../R/AllClasses.R:#setClass("clusteringOutput", representation(
../../R/AllClasses.R:setClass("xvalSpec",
../../R/AllClasses.R:setClass("clusteringOutput", representation(
../../R/RAB.R:setClass("raboostCont", representation(formula="formula", call="call"), contains="list")
../../R/RAB.R:setClass("daboostCont", contains="raboostCont")
../../R/clDesign.R:setClass("prcompObj", contains="prcomp")
../../R/clDesign.R:#setClass("clusteringOutput", representation(
../../R/clDesign.R:#setClass("clusteringSchema", representation(
../../R/pv.R:setClass("projectedLearner", 
../../R/varImp.R:setClass("varImpStruct", representation(method="character",
-->
...
## `xvalSpec` for diverse approaches to cross-validation
...
of partitions), and `fsFunc` (function that performs feature selection
...
-  straight leave-one-out (LOO) -- note the group parameter must be integer; it
...
confuMat(lkran)
```
-  Now do an 8-fold cross-validation with approximate balance
...
## Cross validation with embedded statistical feature selection
...
The `nnet` function returns a structure encoding
predicted probabilities of class occupancy.  We will use
this to enrich the `MLearn`/`nnetI` output to include
